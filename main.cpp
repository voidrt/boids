...
unsigned int ReadComputeShader()
{
    ...
    const auto shaderData = rlCompileShader(shaderSourceCode, RL_COMPUTE_SHADER);
    const auto computeShader = rlLoadComputeShaderProgram(shaderData);
    UnloadFileText(shaderSourceCode);

    return computeShader;
}

unsigned int LoadBoidVAO()
{
    const unsigned int boidVAO = rlLoadVertexArray();
...
int main()
{
    ...
    const unsigned int computeShader = ReadComputeShader();

    const unsigned int position0 = rlLoadShaderBuffer(MAX_BOIDS * sizeof(Vector4), boidPositionArray.data(), RL_DYNAMIC_COPY);
    const unsigned int velocity0 = rlLoadShaderBuffer(MAX_BOIDS * sizeof(Vector4), boidVelocityArray.data(), RL_DYNAMIC_COPY);
    const unsigned int position1 = rlLoadShaderBuffer(MAX_BOIDS * sizeof(Vector4), boidPositionArray.data(), RL_DYNAMIC_COPY);
    const unsigned int velocity1 = rlLoadShaderBuffer(MAX_BOIDS * sizeof(Vector4), boidVelocityArray.data(), RL_DYNAMIC_COPY);

    constexpr Vector4 worldSpace = {WORLD_WIDTH, WORLD_HEIGHT, WORLD_DEPTH, 0.0};

    const Shader boidShader = LoadShader("../Shaders/boid_vertex.glsl", "../Shaders/boid_fragment.glsl");
    const auto boidVAO = LoadBoidVAO();


    while (!WindowShouldClose())
    {
        frameTime = GetFrameTime();
        HandleSimulationControl(camera, isPaused);

        {
            BeginDrawing();
            ClearBackground((Color){2, 5, 0, 255});
            ...
            DrawCubeWires({WORLD_WIDTH / 2, WORLD_HEIGHT / 2, WORLD_DEPTH / 2}, WORLD_WIDTH, WORLD_HEIGHT, WORLD_DEPTH, DARKGRAY);
